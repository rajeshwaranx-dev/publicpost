...
from database import all_users, load_all_posts
...
async def on_startup(app):
    ...
    # ── Restore posted dict from MongoDB ──────────────────────
    ...
    log.info("✅ Bot started. Active users: %d, Restored posts: %d", len(users), sum(len(v) for v in restored.values()))
