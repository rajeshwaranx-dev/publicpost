...
async def build_caption(data: dict, user: dict) -> str:
    ...
    async def make_trinity_batch(file_list: list) -> str:
        ...
        if batch_mode == "batchkey" and trinity_mongo:
            # ── Mode 1: Write to Trinity MongoDB, use batchkey_ link ──
            ...
            # ── Mode 2: Range format get-first*chanid-last*chanid ──
...
async def setbatchmode_command(update: Update, context: ContextTypes.DEFAULT_TYPE):
    """
    /setbatchmode name batchkey|range
    batchkey = writes to Trinity MongoDB (supports random upload order)
    range    = uses get-first*chanid-last*chanid (must upload sequentially)
    """
    args = context.args
    if len(args) < 2:
        await update.message.reply_text(
    ...
    await save_user(user)
    await update.message.reply_text(
        f"✅ Batch mode set to <b>{mode}</b> for <b>{name}</b>",
        parse_mode=ParseMode.HTML,
    )
