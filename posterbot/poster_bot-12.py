...
async def build_caption(data: dict, user: dict) -> str:
    ...
    file_parts = []
    if is_series:
        # Group by episode number → EP01 : 360p | 720p | 1080p
        ep_groups: dict = {}
    ...
    async def make_trinity_batch(file_list: list) -> str:
        """Write msg_ids to Trinity MongoDB and return batchkey_ link."""
        msg_ids = []
        for f in file_list:
            fid = file_id_from_url(f["link"])
        ...
        # Generate short key from sorted msg_ids
        key_str = "_".join(str(m) for m in sorted(msg_ids))
        key     = hashlib.md5(key_str.encode()).hexdigest()[:12]

        # Write to Trinity's MongoDB batches collection
        trinity_mongo_url = user.get("trinity_mongo_url", "")
        trinity_db_name   = user.get("trinity_db_name", "Leechx")
        if trinity_mongo_url:
            try:
                import motor.motor_asyncio as _motor
                _client = _motor.AsyncIOMotorClient(trinity_mongo_url)
                ...
                _client.close()
                log.info("✅ Batch stored in Trinity MongoDB key=%s ids=%s", key, msg_ids)
            except Exception as e:
                log.warning("⚠️ Trinity MongoDB batch store failed: %s", e)

        # Build batchkey_ start param
    ...
    if is_series and files_sorted:
        ...
        # All on one line: 📦 Get all files for: 360p | 720p | 1080p
        q_links = []
        for q in seen_q:
        ...
        if files_sorted:
            batch_link = await make_trinity_batch(files_sorted)
        else:
...
async def setdbchannel_command(update: Update, context: ContextTypes.DEFAULT_TYPE):
    """
    /setdbchannel name -1002049075780
    Set the Trinity filestore DB channel ID for batch link generation.
    """
    args = context.args
    if len(args) < 2:
    ...
    await save_user(user)
    await update.message.reply_text(
        f"✅ DB Channel ID set for <b>{name}</b>: <code>{channel_id}</code>",
        parse_mode=ParseMode.HTML,
    )
...
async def settrinitydb_command(update: Update, context: ContextTypes.DEFAULT_TYPE):
    """
    /settrinitydb name mongodb+srv://... [DbName]
    Set Trinity MongoDB URL so batch links write directly to Trinity's database.
    DbName defaults to Leechx.
    """
    args = context.args
    if len(args) < 2:
        await update.message.reply_text(
    ...
    await save_user(user)
    await update.message.reply_text(
        f"✅ Trinity MongoDB set for <b>{name}</b>\nDB: <code>{db_name}</code>",
        parse_mode=ParseMode.HTML,
    )
